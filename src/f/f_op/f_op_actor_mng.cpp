...
void J3DUClipper_NS_clip(void);
void J3DUClipper_NS_clip_X1_(void);
...
extern u8 lbl_80378880;
extern u8 lbl_8037888C;
extern u8 lbl_80378898;
extern const char lbl_803788C8[0x188]; // f_op_actor_mng::@stringBase0
...
s32 fopAcM_SearchByName(s16 procName, fopAc_ac_c** actor) {
    *actor = (fopAc_ac_c*)fopAcIt_Judge((fopAcIt_JudgeFunc)fpcSch_JudgeForPName, &procName);
    if (*actor == NULL) {
        ...
        if (fpcM_IsCreating(*actor != NULL ? (*actor)->mBsPcId : 0xFFFFFFFF) != 0) {
            *actor = NULL;
        }
        return 1;
    }
...
fopAcM_prm_class* fopAcM_CreateAppend(void) {
    fopAcM_prm_class* params = static_cast<fopAcM_prm_class*>(cMl::memalignB(-4, sizeof(fopAcM_prm_class)));
    if (params != NULL) {
...
#ifndef NM
fopAcM_prm_class* createAppend(u16 param_1, u32 param_2, const cXyz* param_3, int param_4, const csXyz* param_5, const cXyz* param_6, s8 param_7, unsigned int param_8) {
    ...
    if (params == NULL) {
        ...
        if (param_6 != NULL) {
            f32 ten = lbl_80451C00; // temp not necessary when using literal
...
#else
asm fopAcM_prm_class* createAppend(u16, u32, const cXyz*, int, const csXyz*, const cXyz*, s8, unsigned int) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_80019B1C.s"
}
#endif
...
void fopAcM_delete(fopAc_ac_c* pActor) {
    fopAcM_Log(pActor, lbl_803788C8);
    fpcM_Delete(pActor);
}
...
s32 fopAcM_create(s16 pProcName, u16 param_2, u32 param_3, const cXyz* pPos, int param_5, const csXyz* pRot, const cXyz* param_7, s8 param_8, createFunc pCreateFunc) {
    fopAcM_prm_class* params = createAppend(param_2, param_3, pPos, param_5, pRot, param_7, param_8, 0xFFFFFFFF);
    if (params == NULL) {
        return 0xFFFFFFFF;
    } else {
        return fpcSCtRq_Request(fpcLy_CurrentLayer(), pProcName, (stdCreateFunc)pCreateFunc, NULL, params);
    }
}

// fopAcM_create__FsUlPC4cXyziPC5csXyzPC4cXyzSc
s32 fopAcM_create(s16 pProcName, u32 param_2, const cXyz* pPos, int param_4, const csXyz* pRot, const cXyz* param_6, s8 param_7) {
...
void* fopAcM_fastCreate(s16 pProcTypeID, u32 param_2, const cXyz* param_3, int param_4, const csXyz* param_5, const cXyz* param_6, s8 param_7, createFunc param_8,
                            void* param_9) {
    fopAcM_prm_class* prmClass = createAppend(0xFFFF, param_2, param_3, param_4, param_5, param_6, param_7, 0xFFFFFFFF);
    if (prmClass == NULL) {
        return NULL;
    } else {
...
void* fopAcM_fastCreate(const char* pActorName, u32 pActorParams, const cXyz* pActorPos, int param_4, const csXyz* pActorRot, const cXyz* param_6,
                            createFunc param_7, void* param_8) {
    objectNameInfo* nameInfo = dStage_searchName(pActorName);
    if (nameInfo == NULL) {
        return NULL;
    } else {
        return fopAcM_fastCreate(nameInfo->mProcTypeID, pActorParams, pActorPos, param_4, pActorRot, param_6, nameInfo->unkA, param_7, param_8);
    }
}

// fopAcM_createChild__FsUiUlPC4cXyziPC5csXyzPC4cXyzScPFPv_i
s32 fopAcM_createChild(s16 pProcTypeID, unsigned int param_2, u32 pActorParams, const cXyz* pActorPos, int param_5, const csXyz* pActorRot, const cXyz* param_7, s8 param_8,
                           createFunc param_9) {
    fopAcM_prm_class* paramClass = createAppend(0xFFFF, pActorParams, pActorPos, param_5, pActorRot, param_7, param_8, param_2);
    if (paramClass == NULL) {
        return -1;
    } else {
        return fpcSCtRq_Request(fpcLy_CurrentLayer(), pProcTypeID, (stdCreateFunc)param_9, NULL, paramClass);
    }
}

inline float cM_scos(s16 s) {
    return lbl_80439A20[s].cos;
}

inline float cM_ssin(s16 s) {
    return lbl_80439A20[s].sin;
}

inline cXyz projectY(const cXyz& origin, const cXyz& proj, s16 yRot) {
    cXyz out;
    out.x = origin.x + proj.z * cM_ssin(yRot) + proj.x * cM_scos(yRot);
    out.y = origin.y + proj.y;
    out.z = origin.z + proj.z * cM_scos(yRot) - proj.x * cM_ssin(yRot);
    return out;
}

// fopAcM_createChildFromOffset__FsUiUlPC4cXyziPC5csXyzPC4cXyzScPFPv_i
#ifdef NON_MATCHING
s32 fopAcM_createChildFromOffset(s16 pProcTypeID, unsigned int pParentProcID, u32 pActorParams, const cXyz* pActorPos, int param_5, const csXyz* pActorRot,
                                     const cXyz* param_7, s8 param_8, createFunc param_9) {
    struct {
        s32 procID;
    } searchParams;
    searchParams.procID = pParentProcID;
    fopAc_ac_c *parentActor = static_cast<fopAc_ac_c*>(fopAcIt_Judge((fopAcIt_JudgeFunc)fpcSch_JudgeByID, &searchParams));
    // s16 parentRotY = *(s16*)(((int)parentActor) + 0x4de);
    s16 parentRotY = parentActor->getUnk4DC().GetY();
    cXyz tmpPos;
    if (pActorPos == NULL) {
        tmpPos = lbl_80430CF4;
    } else {
    ...
    // cXyz parentPos = *(cXyz*)(((int)parentActor) + 0x4d0);
    cXyz parentPos = parentActor->getUnk4D0();
    // csXyz tmpRotCopy(tmpRot);
    // tmpRotCopy.mY += parentRotY; 
    // csXyz tmpRotCopy(tmpRot.mX, tmpRot.mY + parentRotY, tmpRot.mZ);
    csXyz tmpRotCopy = tmpRot.createRotY(parentRotY); 
    ...
    // cXyz pos;
    // pos.x = *(float*)(((int)parentActor) + 0x4d0) + tmpPos.z * cM_ssin(parentRotY) + tmpPos.x * cM_scos(parentRotY);
    // pos.y = *(float*)(((int)parentActor) + 0x4d4) + tmpPos.y;
    // pos.z = *(float*)(((int)parentActor) + 0x4d8) + tmpPos.z * cM_scos(parentRotY) - tmpPos.x * cM_ssin(parentRotY);
    fopAcM_prm_class* prmClass = createAppend(0xFFFF, pActorParams, &parentPos, param_5, &tmpRotCopy, param_7, param_8, pParentProcID);
    if (prmClass == NULL) {
        return -1;
    } else {
        return fpcSCtRq_Request(fpcLy_CurrentLayer(), pProcTypeID, (stdCreateFunc)param_9, NULL, prmClass);
    }
}
#else
...
void fopAcM_DeleteHeap(fopAc_ac_c* pActor) {
    fopAcM_Log(pActor, (const char*) ((s32)&lbl_803788C8) + 0x42);
...
asm s32 fopAcM_entrySolidHeap_(fopAc_ac_c*, heapCallbackFunc, u32) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A1E8.s"
}

// fopAcM_entrySolidHeap__FP10fopAc_ac_cPFP10fopAc_ac_c_iUl
// fopAcM_entrySolidHeap(fopAc_ac_c*, int (*)(fopAc_ac_c*), u32)
asm s32 fopAcM_entrySolidHeap(fopAc_ac_c*, heapCallbackFunc, u32) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A4B0.s"
}

// fopAcM_SetMin__FP10fopAc_ac_cfff
asm void fopAcM_SetMin(fopAc_ac_c*, f32, f32, f32) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A528.s"
}

// fopAcM_SetMax__FP10fopAc_ac_cfff
asm void fopAcM_SetMax(fopAc_ac_c*, f32, f32, f32) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A538.s"
}

// fopAcM_setCullSizeBox__FP10fopAc_ac_cffffff
asm void fopAcM_setCullSizeBox(fopAc_ac_c*, f32, f32, f32, f32, f32, f32) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A548.s"
}

// fopAcM_setCullSizeSphere__FP10fopAc_ac_cffff
asm void fopAcM_setCullSizeSphere(fopAc_ac_c*, f32, f32, f32, f32) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A564.s"
}
...
asm bool fopAcM_addAngleY(fopAc_ac_c*, s16, s16) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A5DC.s"
}

// fopAcM_calcSpeed__FP10fopAc_ac_c
// fopAcM_calcSpeed(fopAc_ac_c*)
asm void fopAcM_calcSpeed(fopAc_ac_c*) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A60C.s"
}

// fopAcM_posMove__FP10fopAc_ac_cPC4cXyz
// fopAcM_posMove(fopAc_ac_c*, const cXyz*)
asm void fopAcM_posMove(fopAc_ac_c*, const cXyz*) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A660.s"
}

// fopAcM_posMoveF__FP10fopAc_ac_cPC4cXyz
asm void fopAcM_posMoveF(fopAc_ac_c*, const cXyz*) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A6CC.s"
}

// fopAcM_searchActorAngleY__FPC10fopAc_ac_cPC10fopAc_ac_c
asm s16 fopAcM_searchActorAngleY(const fopAc_ac_c*, const fopAc_ac_c*) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A710.s"
}
...
asm s16 fopAcM_seenActorAngleY(const fopAc_ac_c*, const fopAc_ac_c*) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A79C.s"
}
...
asm float fopAcM_searchActorDistance2(const fopAc_ac_c*, const fopAc_ac_c*) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001A914.s"
}
...
asm float fopAcM_searchActorDistanceXZ2(const fopAc_ac_c*, const fopAc_ac_c*) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001AA94.s"
}

// fopAcM_rollPlayerCrash__FPC10fopAc_ac_cfUlffif
// fopAcM_rollPlayerCrash(const fopAc_ac_c*, f32, u32, f32, f32, int, f32)
asm s32 fopAcM_rollPlayerCrash(const fopAc_ac_c*, f32, u32, f32, f32, int, f32) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001AAE0.s"
}

// fopAcM_checkCullingBox__FPA4_fffffff
// fopAcM_checkCullingBox(f32 [][4], f32, f32, f32, f32, f32, f32)
asm bool fopAcM_checkCullingBox(f32[3][4], f32, f32, f32, f32, f32, f32) {
    nofralloc
#include "f/f_op/f_op_actor_mng/asm/func_8001AC40.s"
}

// fopAcM_cullingCheck__FPC10fopAc_ac_c
// fopAcM_cullingCheck(const fopAc_ac_c*)
