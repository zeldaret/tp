...
void* fpcNd_IsCreatingFromUnder(process_node_class* pProcNode) {
    layer_class *layer;
    if (pProcNode != NULL && fpcBs_Is_JustOfType(lbl_80450D40,pProcNode->mBase.mSubType) != FALSE) {
...
s32 fpcNd_Delete(process_node_class* pProcNode) {
  if ((fpcLy_IsDeletingMesg(&pProcNode->mLayer) == 0x0) && fpcMtd_Delete(&pProcNode->mpNodeMtd->mBase,pProcNode) == 0x1) {
