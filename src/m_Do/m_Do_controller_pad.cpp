...
void mDoCPd_c::create(){
    ...
    JUTGamePad_ptr = (JUTGamePad*)new(u32)(sizeof(JUTGamePad));
    if (JUTGamePad_ptr != 0) {
        JUTGamePad_ptr->JUTGamePad(JUTGamePad::Port_1);
    }
    ...
    if (lbl_80450580 == 0) {
        ...
        JUTGamePad_ptr = (JUTGamePad*)new(u32)(sizeof(JUTGamePad));
        if (JUTGamePad_ptr != NULL) {
            JUTGamePad_ptr->JUTGamePad(JUTGamePad::Port_2);
        }
        m_gamePad[1] = JUTGamePad_ptr;
        JUTGamePad_ptr = (JUTGamePad*)new(u32)(sizeof(JUTGamePad));
        if (JUTGamePad_ptr != NULL) {
            JUTGamePad_ptr->JUTGamePad(JUTGamePad::Port_3);
        }
        m_gamePad[2] = JUTGamePad_ptr;
        JUTGamePad_ptr = (JUTGamePad*)new(u32)(sizeof(JUTGamePad));
        if (JUTGamePad_ptr != NULL) {
            JUTGamePad_ptr->JUTGamePad(JUTGamePad::Port_4);
        }
    }
    m_gamePad[3] = JUTGamePad_ptr;
    if (mResetData->field_0x0 == 0) {
        lbl_80451501 = false;
        lbl_804514EC = mDoRst_resetCallBack;
        lbl_804514F0 = 0;
    }
    sAnalogMode = 3;
    PADSetAnalogMode(3);
...
void mDoCPd_c::read(){
    //_savegpr_29();
    gamePad->read();
    if (mResetData->field_0x0 == 0 && mResetData->field_0x8 != 0) {
        JUTGamePad* JUTGamePad_ptr = JUTGamePad::getGamePad(mResetData->pad_index);
        bool error = false;
        if (JUTGamePad_ptr->pad_port != 0xffff && JUTGamePad_ptr->rumble.field_0x10 != 0) { //Make the rumble thing right
            error = true;
        }
        if (error = false) {
            mResetData->field_0x8 = 0;
        }
    }
...
void mDoCPd_c::convert(interface_of_controller_pad* controllerInterface, JUTGamePad* gamePad){
    ...
    controllerInterface->analog_a = (gamePad->buttons.analog_a - lbl_80451A30) * lbl_80451A20;//@3709 * (float)((double)CONCAT44(0x43300000,(uint)(gamePad->buttons).analog_a) - @3713);
    if (controllerInterface->analog_a > lbl_80451A24) {
        controllerInterface->analog_a = lbl_80451A24;
    }
    controllerInterface->analog_b = (gamePad->buttons.analog_b - lbl_80451A30) * lbl_80451A20;//@3709 * (float)((double)CONCAT44(0x43300000,(uint)(gamePad->buttons).analog_b) - @3713);
    if (controllerInterface->analog_b > lbl_80451A24) {
        controllerInterface->analog_b = lbl_80451A24;
    }
    controllerInterface->trigger_left = (gamePad->buttons.trigger_left - lbl_80451A30) * lbl_80451A28;//@3711 * (float)((double)CONCAT44(0x43300000,(uint)(gamePad->buttons).trigger_left) - @3713);
    if (controllerInterface->trigger_left > lbl_80451A24) {
        controllerInterface->trigger_left = lbl_80451A24;
    }
    controllerInterface->trigger_right = (gamePad->buttons.trigger_right - lbl_80451A30) * lbl_80451A28;//@3711 * (float)((double)CONCAT44(0x43300000,(uint)(gamePad->buttons).trigger_right) - @3713);
