...
void dKyr_drawSun(Mtx drawMtx, cXyz* ppos, GXColor& param_2, u8** tex) {
    dScnKy_env_light_c* envlight = dKy_getEnvlight();
    cXyz pos[4];
    cXyz sunpos;
    cXyz spB4;
        ...
        for (int i = 0; i < 8; i++) {
            if (!g_env_light.mpSunLenzPacket->mDrawLenzInSky || i == 0) {
                ...
                if (i < 2) {
                    ...
                    f32 sp5C = 0.12f;
                    spB4.r = ((f32)spD0.r * (1.0f - sp5C)) + ((f32)color_reg0.r * sp5C);
                    spB4.g = ((f32)spD0.g * (1.0f - sp5C)) + ((f32)color_reg0.g * sp5C);
                    spB4.b = ((f32)spD0.b * (1.0f - sp5C)) + ((f32)color_reg0.b * sp5C);
                    ...
                    spB4.a = spAC * (0.5f * (250.0f * spC4) * lenz_packet->mDistFalloff);
                    spB0.a = spAC * (0.5f * (250.0f * spC4) * lenz_packet->mDistFalloff);
                }
                ...
                if (i < 2) {
                    ...
                    var_f31 = 60.0f * (spC8 * scale_dat[i]) * (1.0f - (lenz_packet->mDistFalloff * lenz_packet->mDistFalloff * lenz_packet->mDistFalloff));
                }
...
void drawCloudShadow(Mtx drawMtx, u8** tex) {
    ...
    if (temp_r30->mCount > 0) {
        ...
        for (int i = 0; i < temp_r30->mCount; i++) {
            ...
            if (!(temp_r30->mCloudEff[i].mAlpha <= 0.01f)) {
                sp50.a = 255.0f * (temp_r30->mCloudEff[i].mAlpha * var_f29);
                GXSetTevColor(GX_TEVREG0, sp50);
