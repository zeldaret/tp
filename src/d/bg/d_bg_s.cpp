...
void cBgS_ChkElm::Init() {
    ...
    m_used = false;
    m_actor_ptr = NULL;
    field_0x8 = 0xFFFFFFFF;
}

/* 80074110-80074128 06EA50 0018+00 1/0 0/0 0/0 .text Regist2__11cBgS_ChkElmFP9dBgW_BaseUiPv */
void cBgS_ChkElm::Regist2(dBgW_Base* base_ptr, unsigned int param_1, void* param_2) {
    m_used = true;
    m_bgw_base_ptr = base_ptr;
    field_0x8 = param_1;
    m_actor_ptr = (fopAc_ac_c*)param_2;
}
...
bool cBgS::Regist(dBgW_Base* p_data, unsigned int param_1, void* param_2) {
    if (p_data != NULL && !p_data->ChkUsed() && !p_data->ChkMemoryError()) {
        for (int i = 0; i < 0x100; i++) {
            if (!m_chk_element[i].m_bgw_base_ptr->ChkUsed()) {
                m_chk_element[i].Regist2(p_data, param_1, param_2);
                p_data->Regist(i);
...
#ifdef NONMATCHING
bool cBgS::LineCross(cBgS_LinChk* param_0) {
    param_0->ClearPi();
    param_0->ClrHit();
    param_0->PreCalc();

    for (int i = 0; i < 0x100; i++) {
        if (m_chk_element[i].ChkUsed() && !m_chk_element[i].m_bgw_base_ptr->ChkLock() && !param_0->ChkSameActorPid(m_chk_element[i].field_0x8) && m_chk_element[i].m_bgw_base_ptr->LineCheck(param_0)) {
            param_0->SetActorInfo(i, &m_chk_element[0], m_chk_element[i].field_0x8);
            param_0->SetHit();
        }
    }
...
#pragma optimization_level 0
#pragma optimizewithasm off
asm bool cBgS::LineCross(cBgS_LinChk* param_0) {
    nofralloc
...
bool cBgS::ChkPolySafe(cBgS_PolyInfo const& poly) {
    ...
    if (bg_index < 0 || bg_index >= 0x100) {
        ...
        if (m_chk_element[bg_index].ChkUsed()) {
            return poly.ChkSafe(m_chk_element[bg_index].m_bgw_base_ptr, m_chk_element[bg_index].field_0x8);
        } else {
            return false;
        }
    }
...
// something weird with virtual call
#ifdef NONMATCHING
bool cBgS::GetTriPla(cBgS_PolyInfo const& poly, cM3dGPla* param_1) const {
    int bg_index = poly.GetBgIndex();
    if (bg_index < 0 || bg_index >= 0x100 || !m_chk_element[bg_index].ChkUsed()) {
        return false;
    }
    cM3dGPla pla;
    m_chk_element[bg_index].m_bgw_base_ptr->GetTriPla(poly);
    param_1->mNormal.x = pla.mNormal.x;
    param_1->mNormal.y = pla.mNormal.y;
    param_1->mNormal.z = pla.mNormal.z;
    param_1->mD = pla.mD;
    return true;
}
#else
...
#pragma optimization_level 0
#pragma optimizewithasm off
asm bool cBgS::GetTriPla(cBgS_PolyInfo const& param_0, cM3dGPla* param_1) const {
    nofralloc
...
bool cBgS::GetTriPnt(cBgS_PolyInfo const& poly, cXyz* param_1, cXyz* param_2,
                         cXyz* param_3) const {
...
bool dBgS::GetSpecialCode(cBgS_PolyInfo const& poly) {
    int bg_index = poly.GetBgIndex();
    if (m_chk_element[bg_index].ChkUsed()) {
 ...
 */
bool dBgS::GetMonkeyBarsCode(cBgS_PolyInfo const& poly) {
    int bg_index = poly.GetBgIndex();
    if (m_chk_element[bg_index].ChkUsed()) {
...
bool dBgS::GetUnderwaterRoofCode(cBgS_PolyInfo const& poly) {
    int bg_index = poly.GetBgIndex();
    if (m_chk_element[bg_index].ChkUsed()) {
...
bool dBgS::GetGroundCode(cBgS_PolyInfo const& poly) {
    int bg_index = poly.GetBgIndex();
    if (m_chk_element[bg_index].ChkUsed()) {
...
void dBgS::MoveBgCrrPos(cBgS_PolyInfo const& poly, bool param_1, cXyz* param_2,
    ...
    if (!param_1 || !poly.ChkBgIndex()) {
        ...
        if (m_chk_element[bg_index].ChkUsed()) {
            ...
            if ((!param_5 || base->chkStickWall()) && (!param_6 || base->chkStickRoof()) && base->ChkMoveFlag() && ChkPolySafe(poly)) {
                base->CrrPos(poly, m_chk_element[bg_index].m_actor_ptr, param_1, param_2, param_3, param_4);
            }
        }
    }
}
...
void dBgS::MoveBgTransPos(cBgS_PolyInfo const& poly, bool param_1, cXyz* param_2,
    ...
    if (!param_1 || !poly.ChkBgIndex()) {
        ...
        if (m_chk_element[bg_index].ChkUsed()) {
            ...
            if (base->ChkMoveFlag() && ChkPolySafe(poly)) {
                base->TransPos(poly, m_chk_element[bg_index].m_actor_ptr, param_1, param_2, param_3, param_4);
            }
        }
    }
}
...
void dBgS::MoveBgMatrixCrrPos(cBgS_PolyInfo const& poly, bool param_1, cXyz* param_2,
    ...
    if (!param_1 || !poly.ChkBgIndex()) {
        ...
        if (m_chk_element[bg_index].ChkUsed()) {
            dBgW_Base* base = m_chk_element[bg_index].m_bgw_base_ptr;
            if (base->ChkMoveFlag()) {
                base->MatrixCrrPos(poly, m_chk_element[bg_index].m_actor_ptr, param_1, param_2, param_3, param_4);
            }
        }
    }
}
...
void dBgS_MoveBGProc_Trans(dBgW* param_0, void* param_1, cBgS_PolyInfo const& param_2,
                               bool param_3, cXyz* param_4, csXyz* param_5, csXyz* param_6) {
    cXyz xyz;
...
void dBgS::RideCallBack(cBgS_PolyInfo const& param_0, fopAc_ac_c* param_1) {
...
void dBgS::ArrowStickCallBack(cBgS_PolyInfo const& param_0, fopAc_ac_c* param_1,
                                  cXyz& param_2) {
    u16 bg_index = param_0.GetBgIndex();
    dBgW_Base* base = m_chk_element[bg_index].m_bgw_base_ptr;
    if (base->ChkUsed()) {
...
bool dBgS_CheckBWallPoly(cBgS_PolyInfo const& param_0) {
    cM3dGPla pla;

    if (!dComIfG_Bgsp().GetTriPla(param_0, &pla)) {
        return false;
    }
...
bool dBgS_CheckBGroundPoly(cBgS_PolyInfo const& param_0) {
    cM3dGPla pla;
...
bool dBgS_CheckBRoofPoly(cBgS_PolyInfo const& param_0) {
    cM3dGPla pla;
...
f32 dBgS_GetNY(cBgS_PolyInfo const& param_0) {
    cM3dGPla pla;

    dComIfG_Bgsp().GetTriPla(param_0, &pla);
