...
// extern "C" char const* const d_a_e_tk2__stringBase0;
...
static void e_tk2_find(e_tk2_class* i_this) {
    switch (i_this->mMode) {
    case 0:
        anm_init(i_this, 4, 3.0f, 0x0, 1.0f);
        i_this->mMode = 1;
        break;

    case 1:
        if (i_this->mpMorf->isStop()) {
            anm_init(i_this, 10, 3.0f, 0x2, 1.0f);
            i_this->mMode = 2;
    ...
    case 3:
        anm_init(i_this, 7, 3.0f, 0x0, 1.0f);
        i_this->mMode = 4;
        break;
    ...
    case 10:
        anm_init(i_this, 7, 3.0f, 0x0, 3.0f);
        i_this->mMode = 0xb;
        break;
    ...
    case 0xb:
        ...
        if (i_this->mpMorf->isStop()) {
            ...
            anm_init(i_this, 9, 0.0f, 0x2, i_this->mAnimSpeed);
            i_this->mMode = 0xc;
        }
        break;
...
static void e_tk2_attack(e_tk2_class* i_this) {
    ...
    case 0:
        i_this->mMode = 1;
        anm_init(i_this, 5, 3.0f, 0x00, 1.0f);
        break;
    ...
    case 1:
        ...
        if (i_this->mpMorf->isStop()) {
            anm_init(i_this, 10, 1.0f, 0x02, 1.0f);
...
static int daE_TK2_Execute(e_tk2_class* i_this) {
    ...
    if (i_this->mTKBallSpawned) {
        ...
        if (ball_actor != NULL) {
            ...
            ball_actor->field_0x5ac[0x31C] = 0x0;  // Assumption
        }
        cXyz scale;
        scale.setall(2.0f);
