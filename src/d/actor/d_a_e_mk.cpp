...
static int daE_MK_Draw(e_mk_class* i_this) {
    ...
    if (i_this->field_0x6f8 != 0) {
        ...
        i_this->mShadowKey1 = dComIfGd_setShadow(
            i_this->mShadowKey1, 1, model, &sp38, 1200.0f, 0.0f, i_this->current.pos.y,
            i_this->mAcch.GetGroundH(), i_this->mAcch.m_gnd, &i_this->tevStr, 0, 1.0f,
    ...
    if (i_this->field_0x60c != 0) {
        ...
        if (i_this->field_0x5e0 != 0) {
            ...
            i_this->mShadowKey2 = dComIfGd_setShadow(
                i_this->mShadowKey2, 1, i_this->mpCrownModelMorf->getModel(), &sp38, 500.0f, 0.0f,
...
static void* s_d_sub(void* i_actor, void* i_data) {
    if (((fopAc_IsActor(i_actor) && fopAcM_GetName(i_actor) == PROC_E_DB) &&
         static_cast<e_db_class*>(i_actor)->field_0x66a == 10) &&
...
static void e_mk_move(e_mk_class* i_this) {
            ...
            if (i_this->field_0x718 != 0) {
                ...
                if (i_this->field_0x6fa == 0) {
                    if (cM_rndF(1.0f) < 0.75f) {
                        ...
                        if ((cM_rndF(1.0f) < 0.5f && i_this->field_0x71c >= 3) ||
                            i_this->mDistToPlayer > l_HIO.mPlayerThrowDistMax)
                        {
                            i_this->mMode = 1;
                        } else {
                            i_this->mMode = 0;
    ...
    cLib_addCalc2(&i_this->current.pos.x, i_this->field_0x5b8.x, 1.0f,
                  fabsf(i_this->field_0x6dc.x));
    cLib_addCalc2(&i_this->current.pos.z, i_this->field_0x5b8.z, 1.0f,
                  fabsf(i_this->field_0x6dc.z));
    cLib_addCalc2(&i_this->current.pos.y, i_this->field_0x5b8.y, 1.0f,
                  fabsf(i_this->field_0x6dc.y));
}
...
static int e_mk_e_demo(e_mk_class* i_this) {
    ...
    case 7:
        if (frame == 144) {
            ...
            i_this->current.angle.x =
                -cM_atan2s(sp38.y, JMAFastSqrt(sp38.x * sp38.x + sp38.z * sp38.z));
            cMtx_YrotS(*calc_mtx, i_this->current.angle.y);
            cMtx_XrotM(*calc_mtx, i_this->current.angle.x);
            sp38.x = 0.0;
            sp38.y = 0.0;
...
static void e_mk_r04_demo(e_mk_class* i_this) {
    ...
    case 12:
        ...
        cLib_addCalcAngleS2(&i_this->current.angle.y, i_this->home.angle.y + (u16)-0x8000, 2,
                            0x1000);
        break;
    }
...
static void cam_3d_morf(e_mk_class* i_this, f32 param_2) {
    cLib_addCalc2(&i_this->mCamCenter.x, i_this->field_0xc5c.x, param_2,
                  i_this->field_0xc74.x * i_this->field_0xc84);
    cLib_addCalc2(&i_this->mCamCenter.y, i_this->field_0xc5c.y, param_2,
                  i_this->field_0xc74.y * i_this->field_0xc84);
    cLib_addCalc2(&i_this->mCamCenter.z, i_this->field_0xc5c.z, param_2,
                  i_this->field_0xc74.z * i_this->field_0xc84);
    cLib_addCalc2(&i_this->mCamEye.x, i_this->field_0xc50.x, param_2,
                  i_this->field_0xc68.x * i_this->field_0xc84);
    cLib_addCalc2(&i_this->mCamEye.y, i_this->field_0xc50.y, param_2,
                  i_this->field_0xc68.y * i_this->field_0xc84);
    cLib_addCalc2(&i_this->mCamEye.z, i_this->field_0xc50.z, param_2,
                  i_this->field_0xc68.z * i_this->field_0xc84);
}
...
static void demo_camera_start(e_mk_class* i_this) {
    ...
    case 8:
        ...
        cLib_addCalc2(&i_this->mCamCenter.y, actor1->current.pos.y + i_this->field_0xc8c, 0.2f,
                      100.0f);
...
static void demo_camera_end(e_mk_class* i_this) {
    ...
    case 6:
        cLib_addCalc2(&i_this->mCamCenter.x, i_this->field_0x5e4.x, 0.1f,
                      i_this->field_0xc84 * 20.0f);
        cLib_addCalc2(&i_this->mCamCenter.y, i_this->field_0x5e4.y + 30.0f, 0.1f,
                      i_this->field_0xc84 * 20.0f);
        cLib_addCalc2(&i_this->mCamCenter.z, i_this->field_0x5e4.z, 0.1f,
                      i_this->field_0xc84 * 20.0f);
        cMtx_YrotS(*calc_mtx, STAGE_ANGLE_Y);
        sp2c.x = 800.0f;
        sp2c.y = 100.0f;
        ...
        if (10 <= i_this->mDemoCamTimer) {
            cLib_addCalc2(&i_this->mCamEye.x, i_pos.x, 0.1f,
                          i_this->field_0xc68.x * i_this->field_0xc88);
            cLib_addCalc2(&i_this->mCamEye.y, i_pos.y, 0.1f,
                          i_this->field_0xc68.y * i_this->field_0xc88);
            cLib_addCalc2(&i_this->mCamEye.z, i_pos.z, 0.1f,
                          i_this->field_0xc68.z * i_this->field_0xc88);
            cLib_addCalc2(&i_this->field_0xc88, 1.0f, 1.0f, 0.1f);
        }

        if (i_this->mDemoCamTimer == 40) {
            Z2GetAudioMgr()->seStart(Z2SE_MK_DARK_INSECT_DEATH, &i_this->field_0x5e4, 0, 0, 1.0f,
                                     1.0f, -1.0f, -1.0f, 0);
        }
    ...
    case 0xB:
        ...
        MtxPosition(&sp2c, &i_pos);
        i_pos += STAGE_CENTER_POS;
        cLib_addCalc2(&i_this->mCamCenter.x, i_pos.x, 0.1f,
                      i_this->field_0xc74.x * i_this->field_0xc84);
        cLib_addCalc2(&i_this->mCamCenter.y, i_pos.y, 0.1f,
                      i_this->field_0xc74.y * i_this->field_0xc84);
        cLib_addCalc2(&i_this->mCamCenter.z, i_pos.z, 0.1f,
                      i_this->field_0xc74.z * i_this->field_0xc84);
        sp2c.x = -150.0f;
        sp2c.y = 45.0f;
        sp2c.z = 700.0f;
    ...
    case 21:
        cLib_addCalc2(&i_this->mCamCenter.y, actor->current.pos.y - 20.0f, 0.2f,
                      i_this->field_0xc84 * 200.0f);
        cLib_addCalc2(&i_this->mCamCenter.x, actor->current.pos.x, 0.4f,
                      i_this->field_0xc84 * 200.0f);
        cLib_addCalc2(&i_this->mCamCenter.z, actor->current.pos.z, 0.4f,
                      i_this->field_0xc84 * 200.0f);
        cLib_addCalc2(&i_this->mCamEye.y, player->current.pos.y + 110.0f, 0.1f,
                      i_this->field_0xc84 * 10.0f);
    ...
    case 0x17:
        cLib_addCalc2(&i_this->mCamCenter.x, player->current.pos.x, 0.4f,
                      i_this->field_0xc84 * 200.0f);
        cLib_addCalc2(&i_this->mCamCenter.z, player->current.pos.z, 0.4f,
                      i_this->field_0xc84 * 200.0f);

        if (i_this->mDemoCamTimer == 20) {
            fopAcM_createItemForMidBoss(&player->current.pos, 64, fopAcM_GetRoomNo(i_this), 0, 0, 0,
                                        -1);
            i_this->field_0xc30 = 10;
...
static void* s_ks_sub(void* i_actor, void* i_data) {
    if ((fopAcM_IsActor(i_actor) && fopAcM_GetName(i_actor) == PROC_NPC_KS) &&
        static_cast<npc_ks_class*>(i_actor)->field_0x5b6 == 0)
    {
        return i_actor;
    }
...
static void demo_camera_r04(e_mk_class* i_this) {
    ...
    case 2:
        if (i_this->mDemoCamTimer > 60) {
            cLib_addCalc2(&i_this->mCamCenter.x, i_this->field_0xc5c.x, 0.05f,
                          i_this->field_0xc74.x * i_this->field_0xc84);
            cLib_addCalc2(&i_this->mCamCenter.y, i_this->field_0xc5c.y, 0.05f,
                          i_this->field_0xc74.y * i_this->field_0xc84);
            cLib_addCalc2(&i_this->mCamCenter.z, i_this->field_0xc5c.z, 0.05f,
                          i_this->field_0xc74.z * i_this->field_0xc84);
            cLib_addCalc2(&i_this->mCamEye.x, i_this->field_0xc50.x, 0.05f,
                          i_this->field_0xc68.x * i_this->field_0xc84);
            cLib_addCalc2(&i_this->mCamEye.y, i_this->field_0xc50.y, 0.05f,
                          i_this->field_0xc68.y * i_this->field_0xc84);
            cLib_addCalc2(&i_this->mCamEye.z, i_this->field_0xc50.z, 0.05f,
                          i_this->field_0xc68.z * i_this->field_0xc84);
    ...
    case 3:
        ...
        cLib_addCalc2(&i_this->mCamCenter.y,
                      i_actor->current.pos.y + 130.0f +
                          cM_ssin(i_this->mDemoCamTimer * 0xA00) * 30.0f,
                      0.2f, 50.0f);
    ...
    case 45:
        cLib_addCalc2(&i_this->mCamCenter.z, i_actor->current.pos.z, 0.2f, 50.0f);
        cLib_addCalc2(&i_this->mCamCenter.y,
                      i_actor->current.pos.y + 90.0f +
                          30.0f * cM_ssin(i_this->mDemoCamTimer * 0xA00),
                      0.2f, 50.0f);
...
static void action(e_mk_class* i_this) {
    ...
    if (i_this->field_0x71b != 0) {
        if (i_this->field_0x71b == 2) {
            ...
            fopAcM_effSmokeSet1(&i_this->field_0xc08, &i_this->field_0xc0c, &sp34,
                                &a_this->shape_angle, 2.5f, &a_this->tevStr, 0);

            PSMTXCopy(model->getAnmMtx(33), *calc_mtx);
            MtxPosition(&sp28, &sp34);
            fopAcM_effSmokeSet1(&i_this->field_0xc10, &i_this->field_0xc14, &sp34,
                                &a_this->shape_angle, 2.5f, &a_this->tevStr, 0);
        } else {
            fopAcM_effSmokeSet1(&i_this->field_0xc08, &i_this->field_0xc0c, &a_this->current.pos,
                                &a_this->shape_angle, 2.5f, &a_this->tevStr, 1);
        }
...
static void anm_se_set(e_mk_class* i_this) {
    ...
    if (i_this->mAnm == 40) {
        ...
        if ((i_this->mpModelMorf->checkFrame(21.0f) || i_this->mpModelMorf->checkFrame(31.0f)) ||
            i_this->mpModelMorf->checkFrame(41.0f))
        {
    ...
    if (i_this->mAnm == 19 || i_this->mAnm == 21) {
        for (int i = 0; i < 2; i++) {
            i_this->field_0xc18[i] = dComIfGp_particle_set(i_this->field_0xc18[i], bo_eno_1[i],
                                                           &i_this->current.pos, NULL, 0);
    ...
    if (i_this->mAnm == 21) {
        for (int i = 0; i < 4; i++) {
            i_this->field_0xc20[i] = dComIfGp_particle_set(i_this->field_0xc20[i], bo_eno_0[i],
                                                           &i_this->eyePos, NULL, 0);
...
static int daE_MK_Execute(e_mk_class* i_this) {
    ...
    mDoMtx_stack_c::transS(i_this->current.pos.x, i_this->current.pos.y + i_this->field_0x6d8,
    ...
    if (i_this->field_0x60c != 0) {
        if (i_this->field_0x5e0 == 0) {
            ...
            mDoMtx_stack_c::transM(l_HIO.mCrownPosAdjust.x, l_HIO.mCrownPosAdjust.y,
                                   l_HIO.mCrownPosAdjust.z);
            ...
            mDoMtx_stack_c::transS(i_this->field_0x5e4.x, i_this->field_0x5e4.y,
                                   i_this->field_0x5e4.z);
...
static int useHeapInit(fopAc_ac_c* actor) {
    ...
    i_this->mpModelMorf =
        new mDoExt_McaMorfSO((J3DModelData*)dComIfG_getObjectRes("E_mk", 48), NULL, NULL,
                             (J3DAnmTransform*)dComIfG_getObjectRes("E_mk", 40), 0, 1.0f, 0, -1,
    ...
    if (i_this->mBtp->init(i_this->mpModelMorf->getModel()->getModelData(),
                           (J3DAnmTexPattern*)dComIfG_getObjectRes("E_mk", 55), 1, 0, 1.0f, 0,
                           -1) == 0)
    {
        return 0;
    }

    i_this->mpCrownModelMorf =
        new mDoExt_McaMorf((J3DModelData*)dComIfG_getObjectRes("E_mk", 49), NULL, NULL,
                           (J3DAnmTransform*)dComIfG_getObjectRes("E_mk", 41), 0, 1.0f, 0, -1, 1,
...
static int daE_MK_Create(fopAc_ac_c* actor) {
    ...
    if (phase == cPhs_COMPLEATE_e) {
        ...
        if (strcmp(dComIfGp_getStartStageName(), "D_MN05B") == 0 && dComIfGs_isStageMiddleBoss()) {
            OS_REPORT("中ボスE_MK やられ後なので再セットしません\n");  // Since it's the middle boss
                                                                       // E_MK after being defeated,
                                                                       // I won't reset it.
            return cPhs_ERROR_e;
        }
        ...
        if (uVar1 != 0xFF) {
            if (dComIfGs_isSwitch(uVar1, fopAcM_GetRoomNo(i_this))) {
                OS_REPORT("E_MK やられ後なので再セットしません\n");  // Since it's after the E_MK
                                                                     // was done, I won't reset it.
                return cPhs_ERROR_e;
            }
        }
        ...
        if (fopAcM_GetRoomNo(i_this) == 4) {
            ...
            if (uVar2 != 0xFF && dComIfGs_isSwitch(uVar2, fopAcM_GetRoomNo(i_this))) {
                ...
                i_this->mBabaChildID =
                    fopAcM_createChild(PROC_E_DB, fopAcM_GetID(i_this), uVar3, &sp30,
                                       fopAcM_GetRoomNo(i_this), NULL, NULL, -1, NULL);

                sp30.set(-10.0f, 5114.0f, -4401.0f);
                i_this->mBabaChildID2 =
                    fopAcM_createChild(PROC_E_DB, fopAcM_GetID(i_this), uVar3, &sp30,
                                       fopAcM_GetRoomNo(i_this), NULL, NULL, -1, NULL);
...
static actor_method_class l_daE_MK_Method = {
    (process_method_func)daE_MK_Create,  (process_method_func)daE_MK_Delete,
    (process_method_func)daE_MK_Execute, (process_method_func)daE_MK_IsDelete,
