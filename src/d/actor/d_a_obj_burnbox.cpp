...
int daObjBurnBox_c::Create() {
    static const u16 particle_id[5] = {
        dPa_RM(ID_ZI_S_OBJ_BURN_A), dPa_RM(ID_ZI_S_OBJ_BURN_B), dPa_RM(ID_ZI_S_OBJ_BURN_C), dPa_RM(ID_ZI_S_OBJ_BURN_D), dPa_RM(ID_ZI_S_OBJ_BURN_E),
    };
...
int daObjBurnBox_c::CreateHeap() {
    J3DModelData* modelData = (J3DModelData*) dComIfG_getObjectRes(l_arcName[getType()], l_bmd[getType()]);
...
cPhs__Step daObjBurnBox_c::create1st() {
    ...
    if (phaseStep == cPhs_COMPLEATE_e) {
        ...
        phaseStep = (cPhs__Step)MoveBGCreate(l_arcName[getType()], dzb_id, dBgS_MoveBGProc_TypicalRotY,
...
int daObjBurnBox_c::Draw() {
    ...
    for (u16 index = 0; index < boxModelData->getMaterialNum(); index++) {
        J3DMaterial* matNodeP = boxModelData->getMaterialNodePointer(index);
        J3DGXColorS10* tevColor = matNodeP->getTevColor(0);
...
/* 8046EDD4-8046EDF4 0007B4 0020+00 1/0 0/0 0/0 .text            daObjBurnBox_MoveBGDelete__FP14daObjBurnBox_c */
...
actor_method_class daObjBurnBox_METHODS = {
    (process_method_func) daObjBurnBox_create1st,
    (process_method_func)daObjBurnBox_MoveBGDelete,
    (process_method_func) daObjBurnBox_MoveBGExecute,
    NULL,
