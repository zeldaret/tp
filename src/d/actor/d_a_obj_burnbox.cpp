...
const dCcD_SrcCyl l_cyl_src = {
    {
        ...
        {                                         // mObj
         0,                                       // mFlags
         {{0x0, 0x0, 0x1e}, {0x0, 0x1e}, 0x79}},  // mObj
...
int daObjBurnBox_c::Create() {
    ...
    int index = 0;
    while (index < 5) {
        mpEmitters[index] = dComIfGp_particle_set(particle_id[index], &pos, NULL, &t_scale, 0xff,
...
int daObjBurnBox_c::Execute(Mtx** mtx) {
    f_0x724++;
    if (f_0x724 % 3 == 0 && (--mColor, mColor == -0x14)) {
        fopAcM_delete(this);
    }
...
int daObjBurnBox_c::Draw() {
    ...
    J3DModelData* model = mpModel->getModelData();
    for (u16 index = 0; index < model->getMaterialNum(); index++) {
        J3DGXColorS10* tevColor = model->getMaterialNodePointer(index)->getTevColor(0);
        tevColor->r = mColor;
        tevColor->g = mColor;
        tevColor->b = mColor;
    }
