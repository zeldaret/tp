 ...
 */
// SECTION_DATA void* daNpc_FairySeirei_c::mCutNameList = (void*)&stringBase_80541E58;
char* daNpc_FairySeirei_c::mCutNameList = "Seirei";
...
void daNpc_FairySeirei_c::reset() {
    int iVar1 = (u8*)&mTalk - (u8*)&mInitFunc;
    if (mpMatAnm[0] != NULL) {
...
BOOL daNpc_FairySeirei_c::evtCutProc() {
    ...
    // temp_r31 = &g_dComIfG_gameInfo + 0x4FF8;
    // temp_r3 = getMyStaffId__16dEvent_manager_cFPCcP10fopAc_ac_ci(temp_r31, "\0Seirei" + 1, (fopAc_ac_c *) this, -1);
...
void daNpc_FairySeirei_c::setPrtcls() {
    ...
    for (int i=0; i<3; i++) {
        ...
        loc.y -= 200.0f;
        mEmitters[i] = dComIfGp_particle_set(id[i], &loc, &mCurAngle, &scale);
        if (mEmitters[i] != NULL) {
            mEmitters[i]->setGlobalTranslation(loc);
        }
    }
}
...
void daNpc_FairySeirei_c::_to_FairyCave() {
    ...
    daNpcT_onTmpBit(sTempBit[mCreate]);
...
//                                          int param_5, daNpcT_evtData_c const* param_6,
//                                          char** param_7) {
//     // NONMATCHING
// }
