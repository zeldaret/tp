...
static void lure_cast(dmg_rod_class* i_this) {
    ...
    if (i_this->reel_btn_flags != 0) {
        ...
        ADD_ANGLE(i_this->field_0x75c, 0x1100);
        ADD_ANGLE(i_this->field_0x75e, 0x880);
    } else {
        sp40.x = 0.0f;
        sp40.y = (0.011f + TREG_F(11)) * sp4C.abs();
        ADD_ANGLE(i_this->field_0x75c, 0x2200);
        ADD_ANGLE(i_this->field_0x75e, 0x1100);
    }
...
static void po_action(dmg_rod_class* i_this, f32 param_1) {
    ...
    if (i_this->timers[0] == 1) {
        ADD_ANGLE(i_this->lure_yaw_target, cM_rndFX(5000.0f));
    }
...
static void ground_action(dmg_rod_class* i_this) {
    ...
    if (i_this->acch.ChkGroundHit()) {
        ...
        ADD_ANGLE(i_this->lure_yaw_offset, 3200);
        ADD_ANGLE(i_this->lure_pitch_offset, 4000);
    }
...
static void wd_action(dmg_rod_class* i_this, f32 param_1, wd_ss* i_wd_s) {
    ...
    if (i_this->lure_type == MG_LURE_FR) {
        ...
        if (fabsf(actor->speed.x) > 1.0f || fabsf(actor->speed.z) > 1.0f) {
            ...
            if (sp8 < 0) {
                ADD_ANGLE(i_wd_s->field_0x38, 200);
            } else {
                SUB_ANGLE(i_wd_s->field_0x38, 200);
            }
        }
    }
...
static void lure_catch(dmg_rod_class* i_this) {
    ...
    if (mgfish->mActionPhase < 2 && i_this->play_cam_timer > 25) {
        ...
        if (mgfish->mJointScale > 0.5f) {
            ADD_ANGLE(target, (mgfish->mJointScale - 0.5f) * (20000.0f + DREG_F(19)));
        }
...
static void uki_pl_arm_calc(dmg_rod_class* i_this) {
    ...
    ADD_ANGLE(sp8.y, -15000.0f * i_this->field_0x1508);
    ADD_ANGLE(sp8.z,   3500.0f * i_this->field_0x1508);

    ADD_ANGLE(sp10.y, -4000.0f * i_this->field_0x1508);
    ADD_ANGLE(sp10.z, (3500.0f * i_this->field_0x1508) + ((-11000.0f + WREG_F(8)) * (i_this->field_0x150c * i_this->field_0x1508)));

    if (i_this->action == ACTION_UKI_HIT && i_this->field_0xf60 > 140.0f + JREG_F(14)) {
        ADD_ANGLE(sp8.y, (50.0f + nREG_F(0)) * cM_ssin(i_this->counter * 0x6200));
        ADD_ANGLE(sp8.z, (50.0f + nREG_F(0)) * cM_ssin(i_this->counter * 0x6500));
        daAlink_getAlinkActorClass()->seStartOnlyReverbLevel(Z2SE_AL_ROD_BEND);
    }
...
static void uki_standby(dmg_rod_class* i_this) {
    ...
    if (i_this->field_0x1508 > 0.3f && i_this->play_cam_mode < 5) {
        ADD_ANGLE(i_this->field_0x1418, (-500.0f + VREG_F(3)) * mDoCPd_c::getStickX3D(PAD_1));
    }
...
static void play_camera(dmg_rod_class* i_this) {
    ...
    case 5: {
        ...
        ADD_ANGLE(i_this->field_0x141a, sp40);
