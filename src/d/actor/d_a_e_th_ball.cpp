...
static void normal_move(e_th_ball_class* i_this, s8 param_1) {
    ...
    a_this->speed.y -= 5.0f;
    if (param_1 != 0) {
        ADD_ANGLE(a_this->current.angle.x, 200.0f * a_this->speedF);
    }
...
static void e_th_ball_shot(e_th_ball_class* i_this) {
    ...
    case 2:
        ...
        if (i_this->speedF > 0.0f) {
            ...
            ADD_ANGLE(i_this->current.angle.y, cM_rndFX(4000.0f));
    ...
    if (i_this->speedF > 0.0f && i_this->mAcch.ChkWallHit()) {
        ...
        if (wall_angle != 35) {
            ...
            ADD_ANGLE(i_this->current.angle.y, 0x8000 - (spA * 2));
...
static void e_th_ball_return(e_th_ball_class* i_this) {
    ...
    case 2:
        ...
        if (sp18.abs() < 80.0f + AREG_F(18)) {
            ...
            i_this->speedF = 10.0f;
            i_this->mMode = 3;
            ADD_ANGLE(i_this->current.angle.y, cM_rndFX(6000.0f));
