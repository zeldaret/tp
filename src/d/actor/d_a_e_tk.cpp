...
// SECTION_DATA static void* switchD_807b89d8[13] = {
...
static void e_tk_wait_0(e_tk_class* i_this) {
    ...
    case 0x0:
        anm_init(i_this, 9, 5.0f, 0x2, 1.0f);
        i_this->mNextAnim = 0x1;
        break;
    case 0x1:
        if (i_this->mAngle1[0] == 0) {
            src_pos = i_this->home.pos - i_this->current.pos;
            i_this->mSomeAngle = cM_atan2s(src_pos.x, src_pos.z) + (short)cM_rndFX(8000.0f);
            i_this->mAngle1[0] = (short)(cM_rndF(30.0f) + 30.0f);
        }

        if (i_this->mAngle1[2] == 0 && way_bg_check(i_this, 200.0f) != 0) {
            i_this->mAngle1[2] = 0x28;
            src_pos = i_this->home.pos - i_this->current.pos;
            i_this->mSomeAngle = cM_atan2s(src_pos.x, src_pos.z) + (short)cM_rndFX(2000.0f);
            i_this->mAngle1[0] = (short)(cM_rndF(30.0f) + 30.0f);
        }
        ...
        {
            i_this->mNextAction = 0x1;
            if (pl_check(i_this, l_HIO.m3, 0x4000)) {
...
static void e_tk_find(e_tk_class* i_this) {
    ...
    switch (i_this->mNextAnim) {  // Creates switch jump table in .data
    case 0x0:
    ...
    case 0x1:
        if (i_this->mpMorf->isStop() != 0) {
            ...
            i_this->mAngle1[0] = (cM_rndF(10.0f) + 10.0f);
        }
        break;
    ...
    case 0x4:
        ...
        if (i_this->mpMorf->isStop() != 0) {
            if (i_this->mUnkFlag) {
                ...
                i_this->mNextAnim = 0x0;
                i_this->mAngle1[0] = 0;
                i_this->mAngle1[1] = (short)(cM_rndF(30.0f) + 30.0f);
            }
        }
        break;
    ...
    case 0xc:
        ...
        fVar1 = 1.0;
        fVar2 = 10.0;
        if (i_this->mAngle1[0] == 0 && i_this->mPlayerDistanceLimit > l_HIO.m3 * 1.2f) {
            i_this->mNextAnim = 0x0;
        } else {
            if (i_this->mAngle1[1] == 0 && i_this->mAcch.ChkWallHit() != 0) {
                i_this->mAngle1[1] = (short)(cM_rndF(10.0f) + 20.0f);
                i_this->mSomeAngle = i_this->mPlayerAngleY + (short)cM_rndFX(8000.0f);
            }
        }
    ...
    cLib_addCalc2(&i_this->speedF, l_HIO.m6 * fVar1, 1.0f, fVar2);
    cMtx_YrotS(*calc_mtx, i_this->shape_angle.y);
...
static void e_tk_pathswim(e_tk_class* i_this) {
    ...
    cLib_addCalc2(&i_this->speedF, l_HIO.m5 * fVar1, 1.0f, fVar2);
    cMtx_YrotS(*calc_mtx, i_this->shape_angle.y);
...
static void action(e_tk_class* i_this) {
    ...
    switch (i_this->mNextAction) {  // Creates switch jump table in .data
    case 0:
        e_tk_wait_0(i_this);
        break;
    case 1:
...
static int daE_TK_Execute(e_tk_class* i_this) {
    ...
    mDoMtx_stack_c::scaleM(l_HIO.m2, l_HIO.m2, l_HIO.m2);

    J3DModel* local_b0 = i_this->mpMorf->getModel();
    ...
    MTXCopy(local_b0->getAnmMtx(3), *calc_mtx);

    cStack_94.set(0.0f, 0.0f, 0.0f);
    MtxPosition(&cStack_94, &i_this->eyePos);

    i_this->attention_info.position = i_this->eyePos;
    i_this->attention_info.position.y = i_this->attention_info.position.y + l_HIO.m2 * 35.0f;

    if (i_this->mParticleSpawned) {
        e_tk_ball_class* a_this =
            static_cast<e_tk_ball_class*>(fopAcM_SearchByID(i_this->mpTK_Ball));
        if (a_this != NULL) {
    ...
    MTXCopy(local_b0->getAnmMtx(2), *calc_mtx);
    cStack_94.set(0.0f, 0.0f, 0.0f);

    MtxPosition(&cStack_94, &cStack_a0);
    i_this->mSph.SetC(cStack_a0);
    i_this->mSph.SetR(50.0f * l_HIO.m2);
...
static int useHeapInit(fopAc_ac_c* a_this) {
    ...
    i_this->mpMorf =
        new mDoExt_McaMorfSO((J3DModelData*)dComIfG_getObjectRes("E_tk", 14), NULL, NULL,
                             (J3DAnmTransform*)dComIfG_getObjectRes("E_tk", 9), 2, 1.0f, 0, -1,
