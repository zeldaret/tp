...
static void e_mm_mt_carry(e_mm_mt_class* i_this) {
    ...
    switch(i_this->mMode){
        case 0: 
            ...
            i_this->mMode = 1;
            s16 tmp = 0x4000 + player->shape_angle.y;
            s16 angle_delta_y = i_this->mRotation.y - tmp;
            if (angle_delta_y > 0x4000 || angle_delta_y < -0x4000){
                i_this->mCarryAngle = -0x8000;
            } else {
                i_this->mCarryAngle  = 0;
            }
            void(0); // if exit must jump here first
            break;
    ...
    cLib_addCalcAngleS2(&i_this->mRotation.y, player->shape_angle.y +  0x4000 + i_this->mCarryAngle,
        1, i_this->mCarryAngleSpeed);
    cLib_addCalcAngleS2(&i_this->mRotation.z, (s16)((TREG_S(5) + 0x10000) - 0x8000), 1, i_this->mCarryAngleSpeed);
...
static void e_mm_mt_drop(e_mm_mt_class* i_this) {
    ...
    if((e_mm_hookCheck(i_this)) == 0){
        ...
        case 4:
            if (i_this->mTimer[0] == 0 && i_this->mAcch.ChkWallHit()) {
                ...
                if (wall_angle != 0x23) {
                    ...
                    i_this->mSpin = wall_angle * (TREG_F(6) + -0.3f);
        ...
        i_this->enemy.eyePos = i_this->enemy.current.pos;
        i_this->enemy.attention_info.position = i_this->enemy.current.pos;
        cLib_addCalcAngleS2(&i_this->mRotation.z,(TREG_S(6)+0x10000)-0x8000,1,0x1000);
        cLib_onBit<u32>(i_this->enemy.attention_info.flags, fopAc_AttnFlag_CARRY_e);
        i_this->enemy.attention_info.distances[fopAc_attn_CARRY_e] = '\a';
