        ...
        if (i_this->field_0x9d8.ChkAtShieldHit()) {
            ...
            for (s32 i = 0; i < 4; i++) {
                if (i_this->field_0xb14[i].ChkTgHit() != 0) {
                    ...
                    s32 j = i_this->field_0xff4.mpCollider->ChkAtType(0x10000);
                    if (j != 0) {
                        cVar6 = 1;
                    } else {
                        j = i_this->field_0xff4.mpCollider->ChkAtType(0x60);
            ...
            if (cVar6 == 0 && i_this->field_0x8a0.ChkTgHit() != 0) {
                ...
                if (i_this->field_0xff4.mpCollider->ChkAtType(0x8010060) != 0) {
                    cVar6 = 2;
                } else {
                    i_this->field_0x69c[3] = 6;
                    i_this->field_0x66e = 7;
...
static void e_yd_wait(e_yd_class* i_this) {
    ...
    switch (i_this->field_0x670) {
    case -1:
        s32 iVar3 = i_this->mpMorf->isStop();
        if (iVar3 != 0) {
            i_this->field_0x670 = 0;
        } else {
...
static void e_yd_attack(e_yd_class* i_this) {
    ...
    commonXyz1 = player->eyePos - i_this->actor.current.pos;

    s32 pitch = cM_atan2s(commonXyz1.y,
    ...
    case 1: {
        ...
        i_this->field_0x678 = i_this->actor.home.pos + commonXyz2;
        cLib_addCalc2(&i_this->actor.current.pos.x, i_this->field_0x678.x, 0.1f,
                      i_this->field_0x694 * 20.0f);
        cLib_addCalc2(&i_this->actor.current.pos.z, i_this->field_0x678.z, 0.1f,
                      i_this->field_0x694 * 20.0f);
        cLib_addCalc2(&i_this->actor.current.pos.y, i_this->field_0x678.y, 0.1f,
                      i_this->field_0x694 * 20.0f);
        cLib_addCalc2(&i_this->field_0x694, 1.0f, 1.0f, 0.1f);
        if (i_this->field_0x69c[0] == 0) {
            i_this->field_0x670 = 2;
            ...
            if (i_this->mLeafRoomNo != 0) {
                ...
                MtxPosition(&commonXyz1, &commonXyz2);
                i_this->field_0x678 = i_this->actor.current.pos + commonXyz2;
            } else {
                cMtx_YrotS(*calc_mtx, i_this->actor.shape_angle.y);
                cMtx_XrotM(*calc_mtx, (short)(0x800 - pitch));
                commonXyz1.x = 0.0f;
                commonXyz1.y = 0.0f;
                commonXyz1.z = TREG_F(6) + 350.0f;
                MtxPosition(&commonXyz1, &commonXyz2);
            ...
            i_this->actor.speed.x = fabsf(i_this->field_0x678.x - i_this->actor.current.pos.x);
            i_this->actor.speed.y = fabsf(i_this->field_0x678.y - i_this->actor.current.pos.y);
            i_this->actor.speed.z = fabsf(i_this->field_0x678.z - i_this->actor.current.pos.z);
            i_this->field_0x694 = 0;
            i_this->field_0x9d8.StartCAt(i_this->actor.current.pos);
    ...
    case 2: {
        ...
        cLib_addCalc2(&i_this->actor.current.pos.x, i_this->field_0x678.x, 0.8f,
                      i_this->actor.speed.x * i_this->field_0x694);
        cLib_addCalc2(&i_this->actor.current.pos.z, i_this->field_0x678.z, 0.8f,
                      i_this->actor.speed.z * i_this->field_0x694);
        if (l_HIO.field_0x18 == 1) {
            cLib_addCalcAngleS2(&i_this->actor.shape_angle.z, i_this->field_0x690, 2, 0x2000);
            cLib_addCalc2(&i_this->actor.current.pos.y, i_this->field_0x678.y - 30.0f, 0.8f,
                          i_this->actor.speed.y * i_this->field_0x694);
        } else {
            cLib_addCalc2(&i_this->actor.current.pos.y, i_this->field_0x678.y, 0.8f,
                          i_this->actor.speed.y * i_this->field_0x694);
        }
...
#ifdef DEBUG
                OSReport("E_YD//////////////AT  SET 1 !!\n");
#endif
                i_this->field_0x670 = 5;
                i_this->field_0x694 = 20.0f;
                anm_init(i_this, 8, 2.0f, 2, 1.0f);
                i_this->field_0x69c[0] = 0x78;
                daPy_getPlayerActorClass()->setDkCaught(&i_this->actor);
    ...
    case 5: {
        ...
        if (daPy_getPlayerActorClass()->getDkCaught()) {
            ...
            commonXyz1 = player->current.pos - i_this->actor.home.pos;
            ...
            if (i_this->mLeafRoomNo == 0) {
                ...
                cLib_addCalcAngleS2(&i_this->actor.shape_angle.z,
                    (i_this->field_0x690 + (2000.0f + KREG_F(17)) *
                        cM_ssin(i_this->field_0x66c * (12000 + KREG_S(0)))),
                    2, 0x2000);
                commonXyz1.x = 0.0f;
                commonXyz1.y = (100.0f + XREG_F(2)) - 30.0f;
                commonXyz1.z = -50.0f + XREG_F(3);
                MtxPosition(&commonXyz1, &i_this->field_0x678);
                i_this->field_0x678 += player->current.pos;
                cLib_addCalcAngleS2(&i_this->actor.shape_angle.y, yaw, 2, 0x1000);
            } else {
                pitch = -0x4000;
                i_this->field_0x678 = player->eyePos;
                i_this->field_0x678.y += 70.0f + JREG_F(8);
            }
            cLib_addCalc2(&i_this->actor.current.pos.x, i_this->field_0x678.x, 1.0f,
                          i_this->field_0x694);
            cLib_addCalc2(&i_this->actor.current.pos.z, i_this->field_0x678.z, 1.0f,
                          i_this->field_0x694);
            cLib_addCalc2(&i_this->actor.current.pos.y, i_this->field_0x678.y, 1.0f,
                          i_this->field_0x694);
    ...
    if (i_this->field_0xb10 != 0) {
        ...
        i_this->actor.current.pos.y -= fVar8;
        i_this->actor.old.pos.y -= fVar8;
        i_this->field_0x1058.CrrPos(dComIfG_Bgsp());
        i_this->actor.current.pos.y += fVar8;
        i_this->actor.old.pos.y += fVar8;
    }
    if (i_this->mLeafRoomNo != 0) {
        cLib_addCalcAngleS2(&i_this->actor.shape_angle.x, pitch + 0x8000, 8, 0x400);
        i_this->field_0xb10 = 1;
    } else {
        cLib_addCalcAngleS2(&i_this->actor.shape_angle.x, -pitch, 8, 0x400);
    }
}
...
static void e_yd_chance(e_yd_class* i_this) {
    ...
    case 1:
        cXyz local_2c;
        local_2c.x = cM_ssin(i_this->field_0x66c * (900 + TREG_S(0))) * (60.0f + TREG_F(17));
        local_2c.z = cM_scos(i_this->field_0x66c * (1000 + TREG_S(2))) * (60.0f + TREG_F(17));
        cLib_addCalc2(&i_this->actor.current.pos.x,
            i_this->actor.home.pos.x + local_2c.x,
            0.1f, i_this->field_0x694 * 20.0f);
        cLib_addCalc2(&i_this->actor.current.pos.z,
            i_this->actor.home.pos.z + local_2c.z,
            0.1f, i_this->field_0x694 * 20.0f);
        if (i_this->mLeafRoomNo != 0) {
            cLib_addCalc2(&i_this->actor.current.pos.y,
...
static void e_yd_s_damage(e_yd_class* i_this) {
    switch (i_this->field_0x670) {
        case 0:
            ...
            local_20.x = 0.0f;
            local_20.y = 15.0f;
            local_20.z = -50.0f + TREG_F(16);
            MtxPosition(&local_20, &i_this->actor.speed);
            i_this->field_0x670 = 1;
            i_this->mCreatureSound.startCreatureVoice(Z2SE_EN_DB_V_DAMAGE_S, -1);
        break;
    case 1:
        i_this->actor.current.pos += i_this->actor.speed;
        local_20.x = i_this->actor.current.pos.x - i_this->actor.home.pos.x;
        local_20.z = i_this->actor.current.pos.z - i_this->actor.home.pos.z;
        if (JMAFastSqrt(local_20.x * local_20.x + local_20.z * local_20.z) > 250.0f + nREG_F(8)) {
...
static void e_yd_damage(e_yd_class* i_this) {
    ...
    case 0:
        ...
        break;
    case 1:
        commonXyz = i_this->actor.current.pos - i_this->actor.home.pos;
        if (commonXyz.abs() > 450.0f + TREG_F(15)) {
...
static void action(e_yd_class* i_this) {
    ...
    s16 angleToPlayer = fopAcM_searchPlayerAngleY(&i_this->actor);

    i_this->mAngleToPlayer = angleToPlayer;

    f32 homeDistFromPlayer = (i_this->actor.home.pos - player->current.pos).abs();
    i_this->mHomeDistFromPlayer = homeDistFromPlayer;

    i_this->actor.field_0x566 = 0;
    ...
    if (unkFlag1) {
        ...
        i_this->actor.attention_info.flags = 0x4;
    } else {
        fopAcM_OffStatus(&i_this->actor, 0);
        i_this->actor.attention_info.flags = 0x0;
    }

    cXyz offsetFromHome = i_this->actor.current.pos - i_this->actor.home.pos;
    f32 distFromHome = offsetFromHome.abs();

    i_this->field_0x848 = i_this->field_0x84c * distFromHome * (0.2f + TREG_F(1));
    ...
    cXyz cStack_38;
    cStack_38.set(-30000.0f, 300000.0f, 30000.0f);
    s32 sphIdx = 0;
    for (s32 i = 0; i < 12; i++) {
        *lineMatPos = *jointPos;
        if (i == 3 || i == 5 || i == 7 || i == 9) {
            if (unkFlag2) {
                ...
                i_this->field_0xb14[sphIdx].SetC(*jointPos + cStack_38);
            }
    ...
    setMidnaBindEffect(&i_this->actor, &i_this->mCreatureSound, &i_this->field_0x678, &xyz);
}
...
static int daE_YD_Execute(e_yd_class* i_this) {
    ...
    if (i_this->mLeafRoomNo != 0) {
        i_this->actor.eyePos = i_this->mJointPositions[6 + YREG_F(3)];
...
static s32 daE_YD_Delete(e_yd_class* i_this) {
...
static int useHeapInit(fopAc_ac_c* i_this) {
    ...
    float* lineMatSize = a_this->mLineMat.getSize(0);
    for (int i = 0; i < 0xc; i++, lineMatSize++) {
        *lineMatSize  = (float)XREG_F(0) * 0.1f + 5.0f;
    }
    J3DModelData* modelData = (J3DModelData*)dComIfG_getObjectRes("E_yd", 26);
#ifdef DEBUG
    if (iVar6 == 0) {
        uVar8 = JUTAssertion::getSDevice();
        JUTAssertion::showAssert(uVar8, "", 0xb7f, "");
        OSPanic("", 0xb7f, "");
    }
#endif
