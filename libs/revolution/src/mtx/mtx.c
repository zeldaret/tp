...
void C_MTXIdentity(Mtx m) {
    ASSERTMSGLINE(203, m, "MtxIdentity():  NULL Mtx 'm' ");
    m[0][0] = 1;
    m[0][1] = 0;
    m[0][2] = 0;
    m[0][3] = 0;
...
void C_MTXCopy(const Mtx src, Mtx dst) {
    ASSERTMSGLINE(264, src, "MTXCopy():  NULL MtxPtr 'src' ");
    ASSERTMSGLINE(265, dst, "MTXCopy():  NULL MtxPtr 'dst' ");
    if (src != dst) {
        dst[0][0] = src[0][0];
        dst[0][1] = src[0][1];
        dst[0][2] = src[0][2];
...
void C_MTXConcat(const Mtx a, const Mtx b, Mtx ab) {
    ...
    ASSERTMSGLINE(338, a, "MTXConcat():  NULL MtxPtr 'a'  ");
    ASSERTMSGLINE(339, b, "MTXConcat():  NULL MtxPtr 'b'  ");
    ASSERTMSGLINE(340, ab, "MTXConcat():  NULL MtxPtr 'ab' ");
...
u32 C_MTXInverse(const Mtx src, Mtx inv) {
    ...
    ASSERTMSGLINE(964, src, "MTXInverse():  NULL MtxPtr 'src' ");
    ASSERTMSGLINE(965, inv, "MTXInverse():  NULL MtxPtr 'inv' ");
...
void C_MTXRotRad(Mtx m, char axis, f32 rad) {
    ...
    ASSERTMSGLINE(1446, m, "MTXRotRad():  NULL MtxPtr 'm' ");
    sinA = sinf(rad);
    cosA = cosf(rad);
...
void C_MTXRotTrig(Mtx m, char axis, f32 sinA, f32 cosA) {
    ASSERTMSGLINE(1501, m, "MTXRotTrig():  NULL MtxPtr 'm' ");
    switch(axis) {
        case 'x':
        case 'X':
            m[0][0] = 1;
        ...
        case 'Z':
            ...
            break;
        default:
            ASSERTMSGLINE(1528, FALSE, "MTXRotTrig():  invalid 'axis' value ");
            break;
    }
}
...
void C_MTXRotAxisRad(Mtx m, const Vec* axis, f32 rad) {
    ...
    ASSERTMSGLINE(1676, m, "MTXRotAxisRad():  NULL MtxPtr 'm' ");
    ASSERTMSGLINE(1677, axis, "MTXRotAxisRad():  NULL VecPtr 'axis' ");
...
void C_MTXTrans(Mtx m, f32 xT, f32 yT, f32 zT) {
    ASSERTMSGLINE(1865, m, "MTXTrans():  NULL MtxPtr 'm' ");
    m[0][0] = 1;
    m[0][1] = 0;
    m[0][2] = 0;
    m[0][3] = xT;
...
void C_MTXTransApply(const Mtx src, Mtx dst, f32 xT, f32 yT, f32 zT) {
    ASSERTMSGLINE(1932, src, "MTXTransApply(): NULL MtxPtr 'src' ");
    ASSERTMSGLINE(1933, dst, "MTXTransApply(): NULL MtxPtr 'src' "); //! wrong assert string
...
void C_MTXScale(Mtx m, f32 xS, f32 yS, f32 zS) {
    ASSERTMSGLINE(2007, m, "MTXScale():  NULL MtxPtr 'm' ");
    m[0][0] = xS;
    m[0][1] = 0;
    m[0][2] = 0;
    m[0][3] = 0;
...
void C_MTXScaleApply(const Mtx src, Mtx dst, f32 xS, f32 yS, f32 zS) {
    ASSERTMSGLINE(2069, src, "MTXScaleApply(): NULL MtxPtr 'src' ");
    ASSERTMSGLINE(2070, dst, "MTXScaleApply(): NULL MtxPtr 'dst' ");
    dst[0][0] = (src[0][0] * xS);
    dst[0][1] = (src[0][1] * xS);
    dst[0][2] = (src[0][2] * xS);
    dst[0][3] = (src[0][3] * xS);
...
void C_MTXQuat(Mtx m, const Quaternion* q) {
    ...
    ASSERTMSGLINE(2144, m, "MTXQuat():  NULL MtxPtr 'm' ");
    ASSERTMSGLINE(2145, q, "MTXQuat():  NULL QuaternionPtr 'q' ");
    ASSERTMSGLINE(2146, q->x || q->y || q->z || q->w, "MTXQuat():  zero-value quaternion ");
