...
void GXSetZCompLoc(GXBool beforeTex) {
	GX_SET_REG(__GXData->peCtrl, beforeTex, GX_BP_ZCONTROL_BEFORE_TEX_ST, GX_BP_ZCONTROL_BEFORE_TEX_END);
...
void GXSetPixelFmt(GXPixelFmt pixelFmt, GXZFmt16 zFmt) {
	...
	GX_SET_REG(__GXData->peCtrl, p2f[pixelFmt], GX_BP_ZCONTROL_PIXEL_FMT_ST, GX_BP_ZCONTROL_PIXEL_FMT_END);
	...
	if (zControlRegOld != __GXData->peCtrl) {
		...
		GX_SET_REG(__GXData->genMode, isZ16, GX_BP_GENMODE_MULTISAMPLE_ST, GX_BP_GENMODE_MULTISAMPLE_END);
		__GXData->dirtyFlags |= GX_DIRTY_GEN_MODE;
	}

	if (p2f[pixelFmt] == GX_PF_Y8) {
		GX_SET_REG(__GXData->cmode1, pixelFmt - GX_PF_Y8, GX_BP_DSTALPHA_YUV_FMT_ST, GX_BP_DSTALPHA_YUV_FMT_END);
...
void GXSetFieldMode(GXBool texLOD, GXBool adjustAR) {
	GX_SET_REG(__GXData->lpSize, adjustAR, GX_BP_LINEPTWIDTH_ADJUST_ST, GX_BP_LINEPTWIDTH_ADJUST_END);
